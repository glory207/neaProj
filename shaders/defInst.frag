...
layout (location = 3) out vec4 gNormF;
     ...
     // uniform int count;
      ...
      if(texture(uSampler[t2], pos2.xy ).w < 0.5 ||texture(uSampler[t1], pos.xy ).w < 0.5 ) discard;
      
      vec3 normal = tbn * ((texture(uSampler[t2], pos2.xy ).xyz * 2.0) - vec3(1));
      fragColor = vec4(texture(uSampler[t1], pos.xy ).xyz,1.0);
